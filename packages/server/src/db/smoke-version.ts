...
import { rollbackVersionSteps, getDefaultVersionId, listVersions } from "../versions/service";
import {
  organizations,
  repositories,
  users,
...
async function main() {
  ...
  const commits1 = await db.select().from(versionCommits).where(eq(versionCommits.versionId, versionId));
  const links1 = await db
    .select()
    .from(versionEntityLinks)
    .where(eq(versionEntityLinks.commitId, commits1[0]?.id ?? ""));
  const blobs1 = await db.select().from(endpoints).where(eq(endpoints.repoId, repoId));
  ok("import1 creates 1 commit on main", commits1.length === 1, `${commits1.length}`);
  ...
  const commits2 = await db.select().from(versionCommits).where(eq(versionCommits.versionId, versionId));
  const commit2 = commits2[0];
  const links2 = await db.select().from(versionEntityLinks).where(eq(versionEntityLinks.commitId, commit2?.id ?? ""));
  const blobs2 = await db.select().from(endpoints).where(eq(endpoints.repoId, repoId));
  ok("import2 creates a 2nd commit", commits2.length === 2, `${commits2.length}`);
