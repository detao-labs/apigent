...
//   - repository_members（repoId, userId, role）是访问仓库内容的唯一显式来源
...
export async function getRepoMemberRole(userId: string, repoId: string): Promise<RepoRole | null> {
  const [row] = await getDB()
    .select({ role: repositoryMembers.role })
    .from(repositoryMembers)
    .where(and(eq(repositoryMembers.userId, userId), eq(repositoryMembers.repoId, repoId)))
    .limit(1);
...
export async function getEffectiveRepoRole(
  ...
  const memberRole = await getRepoMemberRole(userId, repoId);
...
async function getRepoAdminOrgIds(userId: string): Promise<string[]> {
  const db = getDB();
  ...
  const ids = new Set<string>();
  for (const r of adminRows) ids.add(r.orgId);
  for (const r of owned) ids.add(r.orgId);
  return Array.from(ids);
}
...
export async function listAccessibleRepoIds(userId: string): Promise<string[]> {
  ...
  const [adminOrgs, memberships] = await Promise.all([
    getRepoAdminOrgIds(userId),
    db
      .select({ repoId: repositoryMembers.repoId })
      .from(repositoryMembers)
      .where(eq(repositoryMembers.userId, userId)),
  ]);
  const ids = new Set<string>(memberships.map((r) => r.repoId));
  if (adminOrgs.length > 0) {
    const repos = await db
      .select({ id: repositories.id })
      .from(repositories)
      .where(inArray(repositories.orgId, adminOrgs));
