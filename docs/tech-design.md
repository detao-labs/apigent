...
4. **Secret Keys are a separate plane.** MCP tools and the external REST API are authenticated with user-issued Secret Keys carrying `mcp:*` / `api:*` scopes; a tenant or admin session role is never sufficient on its own. (Issue / list / revoke ship in §3.10; verification lands with the gateway that first consumes it.)
...
**Status and the open decision.** `repo:manage_mcp` is deliberately deferred: its shape depends on a question that is not settled yet — whether MCP exposes **knowledge retrieval** (the design in [modules/mcp-gateway.md](./modules/mcp-gateway.md): `search_apis` / `get_api_detail` / `get_project_context`) or **interface invocation** (one tool per OpenAPI operation). The starting position is that **the repository is a scope, not a tool**: the tool set is fixed, a repo-level switch decides whether a repository may be retrieved, and the visible range is inherited from `repository_members` plus the Organization's implied roles — so no second authorization model is needed. Interface-level filtering, and Project-scoped retrieval, come later. The trade-offs and the two risks recorded with this decision (user-scoped keys are all-or-nothing; invocation is a separate V2+ design) live in [modules/mcp-gateway.md](./modules/mcp-gateway.md).
...
**Where the two halves live.** _Issuing_ keys is Platform self-service: keys belong to a user (`secret_keys.user_id`), so `/settings/keys` lists, creates and revokes only your own, authenticated by session. _Verifying_ them is not a Platform concern — the callers are machine clients (Cursor / CLI) hitting the MCP Gateway and the future external REST API, and those requests never carry a browser session. Verification therefore belongs in `packages/server` as a shared primitive (`verifySecretKey(rawToken)` → userId + scopes; checks the hash, `revoked_at`, `expires_at` and the required scope) called by the Hono gateway — the same rule as sessions: _auth primitives in `packages/server`, shared by the webapp and the gateway_. Admin does not participate; `admin_super`'s only write capability is `admin:admins:manage`.
...
> **Implementation status:** LLM calls are live — product code (business-context generation, agent runtime) goes through `@apigent/core/ai` (`createLanguageModel(flow)` on the Vercel AI SDK); `@apigent/server/ai` re-exports it for compatibility. The container registers only the `memory` vector store, `local` storage, and the Postgres queue; pgvector, BullMQ and the MCP Gateway are defined in config/types but have no factory yet — `getVectorStore()` (non-`memory`) and `getQueue()` (non-`postgres`/`memory`) fail fast with `not implemented` (see `packages/core/src/di/container.test.ts`).
>
> **Note (2026-09-22, P0-1 decision):** the `LLMProvider` / `EmbeddingProvider` interfaces described in §5.5.4 / §5.5.5 and the container's `getLLM()` / `getEmbedding()` stubs were **removed**. Model access now uses AI SDK's `LanguageModel` / `EmbeddingModel` types directly, with `@apigent/core/ai` as the factory. Embedding / rerank are RAG-specific and ship with `@apigent/rag`. See [modules/rag-package.md](./modules/rag-package.md) §9 A1. Sections below that still show those interfaces are kept as historical design context.
...
| `admin.login`                                                        | `admin_super`              | ✅ wired   | Platform sign-in trail                                                                                                                                  |
| `admin.user_disable` / `admin.user_enable` / `admin.user_delete`     | `admin_super`              | ✅ wired   | Account lifecycle; delete keeps the audit trail with a null actor                                                                                       |
...
- ✅ **Membership, create and delete events are audited** — every membership mutation (`member.*`, `repo.member_*`, `org.transfer`), the `org.create` / `repo.create` bootstrap and `org.delete` / `repo.delete` write their `operation_logs` row inside the same transaction as the business write, via `recordOperation(tx, …)`; read paths are `GET /api/repos/:id/operations` and `GET /api/orgs/:id/operations` (§5.4.7). **Still open:** import detail rows (`operation_log_details`), repository edit, version activate/rollback, MCP toggle, secret keys, and tenant-side repository edit / version switch.
...
- ✅ **The whole admin read surface ships** — `/audit` (platform-level operations, `admin:audit:view`), `/admins` (list / grant by email / revoke, `admin:admins:manage`), the dashboard (`getPlatformStats()`, `admin:stats:view`) and `/users` + `/users/:id` (read-only, `admin:users:view`). Revoking the last admin is refused (`canRevokeAdmin()`), so the deployment cannot be left without someone able to grant admins. **Still open:** nothing on this surface — account lifecycle (`admin:users:disable` / `admin:users:delete`) and `admin.login` both ship.

**Remaining gaps**, in the order they should be closed:

1. **MCP exposure granularity is unresolved** — `org:delete` / `repo:delete` ship, while `repo:manage_mcp` is deferred on purpose: whether MCP exposes knowledge retrieval or interface invocation decides what that switch even means (§3.9, [modules/mcp-gateway.md](./modules/mcp-gateway.md)). Secret-key issuance and revocation can land before that decision; verification belongs to the gateway.
