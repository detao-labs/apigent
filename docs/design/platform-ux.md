...
**组件实现方式（与 `components.schemas` 同构）**：解析器从 `components.*` 提取可复用组件，落库到与 `data_models` 同构的组件表（`version_id` + `repository_id` + `kind` + `name` + JSON payload；`def_type` 为展示型类型提示，如 securityScheme → http / apiKey / oauth2），随版本快照写入，按 `kind` 分组展示。
...
- Tab：概览（统计）/ 成员（邀请、角色变更、移除）/ 仓库 / 操作日志 / 设置。
