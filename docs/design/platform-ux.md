...
| `/repos/new` | 新建仓库向导 | `repo-new.html` | P0 | 创建已实现；OpenAPI 导入后续接入 |
| `/repos/[id]` | 仓库详情（独立布局） | `repo-detail.html` | P0 | 只读详情 + 版本导入已实现；接口列表待接入 |
...
> **异步化规划（后续迭代）**：解析 + 落库为长耗时操作，V0 用同步接口 + 日志打点（`openapi.import.*`，含分段耗时与结果统计）作为基线；后续改为异步任务（队列 Worker，见 `docs/tech-design.zh.md` 异步任务章节）——提交后立即返回任务进度，解析/落库在后台执行，进度与结果通过顶栏消息通知 + 仓库详情状态徽章展示（进行中 / 成功 / 失败可重试）。接口契约保持兼容：提交返回任务 ID 与状态查询入口。
