...
| `/settings` | 设置（账号 / API 密钥 / 偏好 / 更多） | `account-settings.html` | P0 | 账号与密钥已实现；偏好（语言/主题）已实现；更多（通知偏好）待实现 |
...
- **偏好设置**：偏好（界面语言、主题，与顶栏快捷设置同步）、更多（通知偏好为 V0，含导入结果 / 业务上下文 / API 密钥 / 平台公告开关；会话管理、API 偏好为 V1 规划）。
