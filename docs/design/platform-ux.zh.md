...
| 路由 | 页面 | 优先级 | 当前状态 |
| --- | --- | --- | --- |
| `/login`、`/register` | 认证 | P0 | 已实现 |
| `/` | 仪表盘 | P0 | 已实现（需加引导清单） |
| `/repos` | 仓库列表 | P0 | 骨架（数据源待接入） |
| `/repos/new` | 新建仓库向导 | P0 | 表单已实现（后端待接入） |
| `/repos/[id]` | 仓库详情（多 Tab） | P0 | 占位 |
| `/repos/[id]/endpoints` | 接口列表 + 接口详情 | P0 | 未实现 |
| `/repos/[id]/schemas` | 数据模型 | P1 | 未实现 |
| `/repos/[id]/context` | 业务上下文 | P1 | 未实现 |
| `/repos/[id]/versions` | 版本历史与对比 | P1 | 未实现 |
| `/repos/[id]/settings` | 仓库设置 | P1 | 未实现 |
| `/orgs`、`/orgs/new` | 组织列表/新建 | P0 | 已实现 |
| `/orgs/[id]` | 组织详情（成员/仓库） | P1 | 未实现 |
| `/settings` | 设置（账号 / API 密钥 / 偏好 / 更多） | P0 | 独立一级菜单；密钥管理已并入 |
| `/search` | 语义搜索 | P2（V1） | 未实现 |
| `/projects` | 项目（跨仓库聚合） | P2（V1） | 未实现 |
