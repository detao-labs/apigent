...
- RBAC（组织 / 仓库角色继承与覆盖；Admin 体系仅设计，未实现）
- 通知（分类 / 优先级 / 偏好 / 未读角标）
...
- **鉴权覆盖不完整**：`imports` / `contexts` / `versions` 三个 service 未校验调用者角色——任何已登录用户知道 repoId 即可预览/导入版本、读写接口业务上下文、查看版本历史
- **仓库成员管理**：`repo_permissions` 只有读取，没有任何写入入口（仓库级角色覆盖功能不存在）
...
8. 越权防护：非组织成员访问该组织的组织 / 仓库资源返回 403；只读用户执行写操作返回 403；三个待补鉴权的 service（`imports` / `contexts` / `versions`）修复后，任意已登录用户携带他人 `repoId` 的请求必须被拒绝。以「端点 × 角色」表驱动测试覆盖。
9. 仓库成员管理：能在仓库设置页管理 repo 级角色覆盖，且页面区分「继承自组织」与「显式覆盖」两组；`org_owner` 不会被 `repo_viewer` 覆盖降级。
...
6. `org_admin` 的仓库权限边界：是否对齐文档改为 `repo_admin`（可管仓库成员 / 删仓库 / 开关 MCP），还是只看文档保留 `repo_editor`？
7. 仓库角色覆盖是否允许授予**非组织成员**（当前数据模型支持，且有测试覆盖）？建议先限制为组织成员。
