...
1. **key 是"用户全权"。** 现在没有任何机制把一把 key 限制在"只能访问某几个仓库"上——agent 能看什么完全等于 key 所有者能看什么。`scopes` 目前是 `string[]`，将来要加"scope + 仓库白名单"就是 schema 变更。若预期存在"只给某个 agent 看两个仓库"的需求，**key 的模型必须在实现 MCP 之前定下来**，事后很难收回。
