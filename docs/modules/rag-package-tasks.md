...
- [x] **P3-3 按 P0-3 决策调整 `rag.searchStore`**
  ...
  - **完成记录（2026-09-24）：**
    - **形态**：`SearchStoreConfig` 由单一 `PgFtsSearchStoreConfig` 拆成四个内置接口（`PgFtsJiebaSearchStoreConfig` / `PgFtsBigramSearchStoreConfig` / `PgFtsSimpleSearchStoreConfig` / `DisabledSearchStoreConfig`）+ 原有的外部包分支。**为什么不是写成一个接口 + `provider: "pg-fts-jieba" | …` 的联合字面量**：它不是没试过 —— 实测该写法在 `packages/core/src/config/schema.test.ts:24` 直接报 `TS2322: Type 'true' is not assignable to type 'false'`，即 `schema.ts` 的「zod 推断类型 === 手写类型」编译期断言拒绝它。原因：那条断言比对的是**类型同一性**（`<T>() => T extends A ? 1 : 2` 的技巧），而 `{ provider: "a" | "b" }` 与 `{ provider: "a" } | { provider: "b" }` 只是互相可赋值，不是同一个类型。副产品是变体专属配置有了落点（如 jieba 的领域词典标识）。
    - **默认值**：`DEFAULT_RAG_CONFIG.searchStore.provider` = `pg-fts-jieba`；真实 `apigent.config.yaml` 与 `apigent.config.example.yaml` 同步改值（示例 YAML 的注释写明四个取值与各自代价）。
    - **旧值 `pg-fts` 直接退役、不做别名**（fail-fast）。理由：别名会让「这条索引是哪套分词产出的」不可判定，而 `tokenizer_version` 正是靠它决定要不要 REINDEX。实测报错可读：`Invalid discriminator value. Expected 'pg-fts-jieba' | 'pg-fts-bigram' | 'pg-fts-simple' | 'none' | 'package'`（path `rag.searchStore.provider`）。
    - **新增一条跨字段校验（本任务额外做的，需要用户知情）**：`retrievalMode: sparse-only` + `searchStore.provider: none` 在启动期拒绝（该组合保证空结果且不报错，属最难查的一类故障）；反向组合 `hybrid` + `none` **允许**，语义是退化为 dense-only —— 这正是验收②「不报错」要保住的行为。实测报错文案：`retrievalMode "sparse-only" needs a sparse store, but searchStore.provider is "none", so every query would return an empty result set. Use "pg-fts-jieba" (or "pg-fts-bigram" / "pg-fts-simple"), or set retrievalMode to "dense-only".`（path `rag.retrieval.retrievalMode`，且是该配置下唯一的 issue）。之所以放在 P3-3 而不是 P5-1：定案 A3 把「取值的语义」定在这里，跨字段校验只是把语义写成可执行断言，且它不依赖任何尚未存在的阶段。
    - **验收①（fail-fast）落在两层**：schema 层（4 个内置值均通过；`pg-fts` 与未知值被拒）+ YAML 层（`file-loader.test.ts` 实测：把 `searchStore.provider: pg-fts` 写进配置文件，`loadConfig()` 抛错且消息含 `rag`）。
    - **验收②（`none` ⇒ dense-only）落在配置层**：`none` 可加载、`hybrid` + `none` 不报错、`sparse-only` + `none` fail-fast 三条各有断言。⚠️ **诚实的边界**：运行期「`RetrieveResult.strategy` 实际为 dense」这一半**尚不可验证** —— 稀疏阶段（`SparseIndex`）P4-5 才存在，当前管线本来就只有稠密一路。该证明随 P4-5 / P5-1 一起补，不是「已通过」。
    - **新增漂移守卫测试**（顺带）：`file-loader.test.ts` 加载**提交进版本库的** `apigent.config.example.yaml`（临时目录 + 伪 `.env`），断言能通过 schema 且 `searchStore.provider === "pg-fts-jieba"`。这条把「配置类改动一律四处同步」从约定变成机械保障 —— 示例 YAML 漂移是用户最先撞上的失败。只覆盖 example 文件，不覆盖用户可改的 `apigent.config.yaml`（后者在本地可能处于实验状态，不该让单测因此变红）。
    - **本次未做（属 P4-5）**：`pg-fts-bigram` / `pg-fts-simple` 的 tokenizer 实现与稀疏读写路径。因此今天四个取值的运行时行为**完全一致**（都不驱动任何代码）—— 这是既有状态（示例 YAML 的 🟡 标注保留了），不是本次引入的静默降级。
    - 文档同步：`rag-package.md` §5 阶段清单、§9 A3 连带影响（改为已落地）、§12 配置变更清单（标 ✅）；`tech-design.md` + `tech-design.zh.md` 两处配置样例的 `pg-fts` → `pg-fts-jieba`。
    - 验证：`pnpm -r typecheck` / `pnpm -r lint` / `pnpm -r test` 全绿（core 73 → 85 例，全仓 310 例）；`pnpm db:check` 通过（它经 `loadConfig()` 读真实 `apigent.config.yaml`，等于顺带验证了真实配置在新枚举下仍能加载）。

- [x] **P3-4 chunk 内容寻址改造 + `knowledge_chunk_links` 建表**（按 P0-4）
  - 交付（`knowledge_chunks` 是 0 行，本次改造零成本）：
    ...
    2. 新增 **`knowledge_chunk_links(commit_id, chunk_id)`**，主键 `(commit_id, chunk_id)`，~~索引 `(commit_id)`~~ → **索引 `(chunk_id)`**（见下方记录：主键已覆盖 commit 方向，GC 需要的是反方向）；外键指向 `version_commits` / `knowledge_chunks`。
    3. 新增查询所需索引：向量 HNSW、`search_vector` GIN 保持不变；~~补 `knowledge_chunks(repository_id, chunk_key)`~~ → **不补**（唯一索引前缀已覆盖，见记录）。
  ...
  - **完成记录（2026-09-24）：**
    - **迁移**：`packages/server/drizzle/0006_rag_chunk_content_addressing.sql`（8 条语句），**已在本机 Docker PG（`localhost:5433/apigent`）执行**：`pnpm db:migrate` → 记账 7 → 8 行；`db:check` 通过；再 `generate` 报 `No schema changes`（快照同步）。**本次 drizzle-kit 生成结果无需任何手工修**（P3-2 那两处坑——顺序错、DROP COLUMN 静默丢 GIN 索引——在这里都不成立：新增列不存在，DROP 的是 `version_id`，无索引引用它；预演时已逐项确认 GIN/HNSW 索引存活）。
    - **动真库前的预演**（沿用 P3-2 的习惯）：把 8 条语句在一个事务里跑完，检查 `version_id` 已消失、links 列/主键/索引正确、`knowledge_chunks_search_vector_gin_idx` 仍在，再 `ROLLBACK`；回滚后列与表均复原。预演通过后才跑 `db:migrate`。
    - **两处相对任务文本的有意偏差（索引宁少勿滥）**，均以 `EXPLAIN` 实测为据：
      1. **不建 `knowledge_chunk_links(commit_id)` 单列索引** —— 主键 `(commit_id, chunk_id)` 的 btree 前缀已服务 `WHERE commit_id = $1`（实测走 Bitmap Index Scan on `..._commit_id_chunk_id_pk`）。反而补了**反方向**的 `knowledge_chunk_links_chunk_idx (chunk_id)`：对账式 GC 要判断「这个 chunk 还有没有别的 commit 引用」（`NOT EXISTS`），没有它每次 GC 都是对 links 的顺序扫描（实测走 `knowledge_chunk_links_chunk_idx`）。
      2. **不补 `knowledge_chunks(repository_id, chunk_key)` 非唯一索引** —— 唯一索引 `(repository_id, chunk_key, content_hash)` 的前缀即可服务该查询（实测 `Index Scan using knowledge_chunks_repository_key_hash_idx`，Index Cond 两个等值条件都在）。同列前缀索引是纯冗余（写放大 + 空间）。
      - 唯一索引本身**保留原名空间但改了列**：`knowledge_chunks_repository_key_idx` → `knowledge_chunks_repository_key_hash_idx`（`(repository_id, chunk_key, content_hash)` 唯一）。改名是有意的：旧名字会被读成「repo 内 chunk_key 唯一」，而新语义允许同一 chunk_key 有多个 content_hash 行。
    - **语义验收（事务内 + SAVEPOINT，全部 ROLLBACK，事后表仍 0 行）**：① 同一 `(repo, chunk_key)` 两个不同 `content_hash` 可并存；② 跨仓库同 `key` + 同 `hash` 允许（`repository_id` 是 chunk 身份的一部分，「绝不跨仓库共享」在结构上成立）；③ 同 `(repo, chunk_key, content_hash)` 重复 → `23505`；④ links 主键重复 → `23505`；⑤ links 的 `commit_id` / `chunk_id` 指向不存在的行 → `23503`（**注意**：验证 FK 必须先把 `session_replication_role` 从 `replica` 改回 `origin`，否则 FK 触发器被关、用例会假通过 —— 我第一次就踩了这个坑）；⑥ 同一 chunk 可被两个 commit 引用（links 多对多）；⑦ 生成列仍工作：写 `search_text` 后 `search_vector = '创建':1 '发货单':2`，`@@ to_tsquery('simple','发货单')` 命中。
    - **清理顺序**：`repo-deletion.ts` 在删 `knowledge_chunks` 之前先删 links（按 chunk 子查询一次、按 commit 子查询一次 —— 正常情况下两个集合一致，双删是有意的防御：任一不一致都在这里清干净，而不是让外键把整个仓库删除挡死）；文件头依赖顺序注释已更新。`org-deletion.ts` **未改**并给出理由：它要求「组织下仓库数为 0」，而按不变量 chunk 只能被同仓库的 commit 引用 ⇒ 此时 links 必为空；若真有残留，删 chunk 会因外键失败（fail-closed），不会静默留下越权数据。
    - **schema 与测试**：`knowledge.ts` 去掉 `versionId`、新增 `knowledgeChunkLinks` 表，`schema/index.ts` 导出；`packages/server/src/db/index.test.ts` 补 links 两列与 `contentHash` 的导出断言。
    - 验收中「两个版本共存只返回活跃 commit」「回滚零重索引」「越权 chunk 在 SQL 层排除」三条**属 Phase 4/5**（需要写入路径与检索 SQL），本次只把它们的结构前提落地并留了证据；不是「已通过」。
...
- [x] **P4-1 `pg-document-source`**
  ...
  - **完成记录（2026-09-24）：**
    - **新增端口 `SqlExecutor`**（`packages/rag/src/contracts/ports.ts`，从 `@apigent/rag/contracts` 导出）：只有一个 `query(sql, params) => Row[]`。这是 P0-6/§2.3 的落地 —— rag 不依赖 `pg` / `drizzle`，连接池生命周期归宿主；单测因此可以用「返回固定行」的假执行器，不连库。
    - **适配器落点 `packages/rag/src/adapters/`**（新 subpath `@apigent/rag/adapters`）：`pg-document-source.ts`（六段 SQL + 组装）与 `document-text.ts`（渲染规则）。`pgDocumentSourceProvider` 是 `RagDocumentSourceFactory` 形态的包装，从 `deps.sql` 取句柄，取不到就抛 `RagConfigError`（启动期可读错误，而不是运行期 `undefined is not a function`）。
    - **产出四个层级**：L0 `project:overview`（仓库自述 + `capability_context` + components/securityScheme 渲染成「认证方式」）、L1 `tag:<tag>`（成员接口清单，`parentId` 指向 L0）、L2 `endpoint:<METHOD>:<path>`（summary/description/参数/响应/业务上下文；`parentId` 指向 L1）、L3 `schema:…:request` / `schema:…:response`（含 `$ref` 展开的模型定义）、L3 `rules:<METHOD>:<path>`（业务约束 / 副作用 / 使用场景）。层级顺序固定，便于逐字断言与可复现。
    - **「有 / 无业务上下文」是验收核心**：无业务上下文的接口**仍产出 L2 + L3 schema**（可检索），只是没有 `rules` 文档、L2 正文里也不出现 Capability/Intent 段落；那条路径有专测。
    - **几个刻意的设计选择**：① `$ref` 指向的 `data_models` **内联展开**进 L3 正文（不展开则字段名永远召不回，这是最容易被忽略的召回缺口）；② **不截断**（要控长度是 chunker P4-2 的事，静默截断是最难发现的检索退化）；③ 没有 tag 的接口归入 `tag:untagged` 而不是丢掉，且 L2 的 `parentId` 与之一致（否则 L2 会成为唯一没有 parent 的层级，上下文扩展断链）；④ 标签用英文、内容保留原语言，`lang` 由**内容判定**（含 CJK → zh）而不是配置。
    - **给 P4-6 的两条明确约束（写进代码文件头）**：① `IndexRequest.endpoints` 过滤只作用于 endpoint 级文档，project / tag 始终全量产出（它们不因过滤而变，content_hash 相同会被复用）⇒ **部分重索引时删除范围必须同样收窄**，否则对账会把其余接口的 chunk 当成「不在期望集合」删掉；② 已删除接口的残留靠对账删除解决，不是这里。
    - **验收证据**：rag 单测 153 → 187 例（适配器 20 例 + 渲染规则 14 例）。**六段 SQL 全部在真实本地 PG 上跑通**：临时脚本（用假执行器把适配器**实际发出的 SQL 逐字导出**，再用纯 node 在事务里播种 11 张表的样本 → 执行 → 回滚）验证了列名/别名/`any($1::text[])`/join 与 `business_contexts` 的字段映射，回滚后 `repositories` 归 0。为什么不用 vitest 连库：vitest 的 `require` interop 对 `pg` 返回的对象无法 `new Client()`（实测），而给 rag 加 `pg` 依赖是明确的架构禁忌。
    - 文档同步：`rag-package.md` §2.2 subpath 表（补 `/stages` `/adapters`，修正 `/adapters/*`，标注 `/eval` 尚未落地）+ §4.4 补三条实现约定。
    - 诚实边界：`RagDocumentSource` 目前在**流水线里还没有被消费**（Phase 2 的 `service.ts` 用的是注入的 documentSource 工厂，但注册与装配在 P6-1）；`chunk_key` 的最终形态与对账式删除分别属 P4-2 / P4-6。

- [x] **P4-2 chunker（`hierarchical`）**
  - 交付：L0 project / L1 tag / L2 endpoint / L3 schema+rules 分块；中英双 chunk。
  - 验收：`chunk_key` 稳定且能精确算出期望集合（对账的前提）；同输入重复跑结果幂等。
  - **完成记录（2026-09-24）：**
    - **「中英双 chunk」按用户定案（选项 1）落地为「不翻译」**：一个逻辑单元产出一块，语言由内容判定（P4-1 已如此）。中英互查交给 `text-embedding-v4` 的跨语言能力；等 Phase 7 有 golden set 证明「中文查询召不回英文 chunk」再考虑引入翻译（那时要新增 LLM flow，并决定翻译失败的降级语义）。任务原文的「中英双 chunk」由此**改写为「语言是内容属性」**。
    - **`chunk_key` 规则（对账锚点，唯一一处定义）**：未切分 = `document.id`；切分后 = `{document.id}#{part}`（0-based）。**修订 P0-4 的 `{level}:{method}:{path}:{lang}`** —— 既然不翻译，`lang` 就是**内容属性**而不是身份：放进 key 会带来两个实际代价（① 语言按内容判定，边界文本上可能翻转 ⇒ key 抖动 ⇒ 每轮「删一个插一个」；② 源文档换语言会留下孤儿 key）。现在语言变化只改 `content_hash` ⇒ 命中内容寻址、原地更新。**将来若真做翻译，key 必须重新带上语言后缀**（否则同一逻辑单元会有两行同 key 不同 hash、检索时重复）。
    - **契约**：新增 `Chunker` 端口与 `RagChunk`（`chunkKey` + `document` + `part`/`parts`）到 `contracts/stages.ts` —— key 由分块器算而不是让调用方从 `document.id` 猜，「期望集合」与「实际写入的 key」因此不可能各算各的。
    - **实现**（`packages/rag/src/stages/chunker.ts`，新 subpath 导出）：`hierarchicalChunker`（结构感知）、`fixedChunker`（硬切对照组）、`chunkerForStrategy`（枚举 → 实现的**单一映射点**）。`DEFAULT_MAX_CHARS = 2000` **不进配置** —— 设计文档明写「过早开放会诱使人在分块上过度调参」，所以只留枚举、参数留在代码里。
    - **切分规则**：预算 = 上限 − 标题行；优先在 section 边界（`Summary:` 这类渲染约定的起始行）装箱，每块**都重复标题行**（否则第二块起没有主语）；单个 section 超预算时退到**按行**装箱；只有单行本身超预算才硬切，且**只有第一片带标题**——往行中间插标题会把内容改坏（把一段 JSON 从中间截断再塞一行标题进去）。原则三条：不丢内容、不切碎语义、每块自带标题。
    - **管线接线**：`RagServiceOptions.chunker` **必填**。为什么不给「缺省 = 不分块」：那等于让分块在忘记注入时静默消失，超长文档会被 embedding 静默截断。不放进注册表是因为 `rag.chunkStrategy` 是枚举（没有 `package`/`options` 的位置）；管线**不能 import stages**（P2-8 的 lint 规则），所以映射由宿主做，P6-1 接线时写 `chunkerForStrategy(config.rag.chunkStrategy)`。
    - **F5 收口**：`ChunkStrategyName` 与 core 的 `ChunkStrategy` 之间加了**编译期同一性断言**（`chunker.test.ts`），两份枚举从此不可能悄悄漂移。
    - 切分信息进 `metadata.part` / `metadata.parts`（不进 key）：排障时要能回答「这条为什么只有半截」。
    - **验收证据**：chunker 12 例（幂等逐字相等、期望集合可从 id 精确算出、part 连续、每块带标题、section 不重复不丢失、超长 section 按行切、单行硬切兜底且不丢字符、空输入、fixed 与 hierarchical 结果不同、策略映射覆盖枚举）+ 管线 3 例（超长文档 → 多块写入且 span 的 `resultCount` = 块数；重复索引块数不变；未超长文档行为与接线前完全一致）。rag 187 → 202 例，全仓 388 例全绿。
    - 诚实边界：`rag.chunkStrategy` 的「配置 → 实现」映射**尚未接线**（属 P6-1）；`fixed` 是对照组不是生产推荐；真正的 token 级长度控制要等 P4-3 的 embedder 回来报 token 数（现在是字符预算，保守但有界）。

- [x] **P4-3 embedding provider（qwen 起步）**
  - 交付：基于 P1-1 出口的 embedding 实现 + token 统计。
  - 验收：批量 embedding 有并发 / 批次控制；失败时给出明确错误而非静默截断。
  - **完成记录（2026-09-24）：**
    - **落点**：`packages/rag/src/adapters/openai-compatible-embedder.ts`，导出两层 —— `openAICompatibleEmbedder()`（通用：任意 OpenAI 兼容 provider + 批次/并发/校验）与 `qwenEmbedder()`（工厂：用 `@apigent/core/ai` 的 `createOpenAICompatibleProvider()` 造模型，身份 `qwen:<model>`）。**这是 P0-1 的分工落地**：调用模型服务的能力在 core，embedding 语义在 rag。
    - **依赖**：`packages/rag` 新增 `ai`（`^7.0.62`，与 core 同版本）。这是 §2.2 早就写明的 rag 依赖集（ai / zod / @node-rs/jieba）。
    - **由适配器负责、不推给调用方的三件事**：
      1. **批次**（`maxBatchSize` 默认 10，DashScope 上限 25 留余量）与**并发**（`concurrency` 默认 2，自研有界并发 map —— 不是 `Promise.all` 分批，那样每批要等最慢的一个、并发度还会变成 batchSize×concurrency）。**并发只影响执行顺序，返回顺序与入参逐条对应**（错位是致命静默错误）。
      2. **维度校验**（P0-2）：逐条校验 `vector.length === 1024`，不符直接抛错 —— 不能让别的维度静默混进 `vector(1024)` 列。
      3. **失败带上下文**：`RagDependencyError` 里写明「模型 + 第几批/共几批 + 本批条数」，且**不吞异常、不返回部分结果**（部分成功 = 索引缺一块，而对账会以为「本来就没有」）。
    - **维度是怎么生效的**：`dimensions` 通过 AI SDK 的 providerOptions 命名空间传（`{ [transport.name]: { dimensions } }`）。真 API 实测请求体为 `{"model":"text-embedding-v4","input":[…],"encoding_format":"float","dimensions":1024}` —— 确认插件把该参数带到了线上。
    - **验收证据（离线）**：13 条单测 —— 批次切分（25 条 → 10/10/5）、并发上限（6 批 × 1 条、并发 2 → 实测最大在飞 = 2）、顺序保持、空输入不发请求、token 聚合、providerOptions 透传、provider 报错带模型/批次/条数、向量数不一致拒绝（`refusing to continue`）、维度不符拒绝（768 ≠ 1024）、构造期参数校验、qwen 身份串。
    - **验收证据（真 API 冒烟）**：用本机 `DASHSCOPE_API_KEY` 实调。⚠️ **`text-embedding-v4` 返回 `AllocationQuota.FreeTierOnly`（免费额度用尽）** —— 因此向量层面的端到端用 `text-embedding-v3` 验证（同账号有额度）：3 条文本 → 3 个向量、**全部 1024 维**、`tokens = 37`、耗时 296ms。
      - 顺带得到一个**对 P4-2 决策的直接支持**：跨语言相似度 zh「退款接口」↔ en「Refund endpoint」= **0.7412**，而同语言不同主题 zh「退款」↔ zh「发货单」= **0.5851** —— 同义不同语言比同语言不同主题更近，说明「不翻译、靠跨语言 embedding」在当前模型上站得住。
      - v4 的失败路径也顺便验证了错误语义：`RagDependencyError: embedder: qwen:text-embedding-v4 failed on batch 1/1 (3 texts). Underlying error: Free quota exhausted…`（带上下文、cause 保留）。
    - 诚实边界：默认模型仍是 `text-embedding-v4`（P0-2 指定），但**本机账号缺额度**，所以「v4 的向量质量」尚未验证，需要换有额度的 key 再跑一次冒烟；`maxBatchSize` / `concurrency` 暂不进配置（同 P4-2 的理由：避免过度调参），需要时再开槽位。
    - rag 202 → 215 例；全仓 401 例。

- [x] **P4-4 pgvector 稠密索引适配器**
  ...
  - **完成记录（2026-09-24）：**
    - **决策（用户定案）**：**引入配置槽 `rag.retrieval.minScore`，默认不写 = 不设阈值**。理由：合适的值只能由 golden set 定（Phase 7），今天拍一个数字等于悄悄掐掉召回，而「结果少了几条」比「多返回几条低分」更难发现。四处同步：`types.ts`（可选字段 + 语义与理由）、`schema.ts`（`-1..1`，越界即配置错误 —— 写 2 会让你以为一切正常而实际永远空）、`defaults.ts`（有意不设默认值并写明原因）、示例 YAML（注释掉并说明）。core 99 → 103 例（新增 4 条：缺省合法 / 合法范围 / 越界拒绝 / 非数字拒绝）。
    - **阈值怎么传进查询**：作为 `DenseQuery.minScore`（**不是** 调用方的 `filters`）—— 阈值是部署策略，不是这一次查询的过滤条件；由检索侧从配置注入（P5-1）。适配器把它做成 `1 - (c.embedding <=> $1::vector) >= $n`，与权限 / 版本 / filters 一起在 `ORDER BY` / `LIMIT` **之前**。
    - **适配器**（`adapters/pgvector-index.ts`）：`upsert` / `search` / `unlink` / `dropRepository` / `size` 全部落地，用注入的 `SqlExecutor`（与 document source 共用一个句柄）。
      - **写入 = 内容寻址**：`insert … on conflict (repository_id, chunk_key, content_hash) do update`，行 id 由 `sha256(repo + chunkKey + contentHash)` 确定性生成（幂等、可从三元组反推）。`content_hash` 由**正文**算（sha256）—— 同 key 不同内容 = 两行，这正是「回滚零重索引」的结构基础。
      - **link 用 `insert … select … on conflict do nothing`**：chunk 行可能是本次或上次插入的，从 `knowledge_chunks` 反查 id 比在应用侧猜更可靠。
      - **版本收窄用 `exists` 而不是 JOIN**：一个 chunk 可能被多个 commit 引用，JOIN 会产生重复行、白白吃掉 `LIMIT` 配额。
      - **`fields` 摊平进 `metadata`**：表里没有 method/path/tag 列，而 `RetrievalFilters` 必须在 SQL 里过滤，所以 `metadata->>'method'` / `metadata->'tags' ?| …` / `metadata->>'path' like …` 是唯一能保住「过滤在 SQL 内」的存法（P4-1 的 `fields` 由此不丢）。
      - **`organizationId` 缺失时回落查 `repositories` 并缓存**（快照列 NOT NULL）；查不到就抛可读错误，不静默写坏。
      - **分数转 number**：pg 以字符串返回 `real`，不转换会让「分数是字符串」这种问题只在排序时暴露。
    - **验收证据（离线）**：24 条单测 —— 冲突目标 / content_hash 与 id 随内容变化但同内容幂等 / 每 commit 一条 link / fields 摊平 / org 回落与缓存 / 维度与 embedding_model 与空正文的写入前拒绝 / 权限与版本条件都在 `ORDER BY` 之前且无「先取回再过滤」形态 / 空 scope 不发 SQL / 无 commit 时不加 link 条件 / minScore 与 filters 进 SQL / 只召回当前模型 / 行到 `DenseHit` 的映射 / unlink 的 GC 带 `NOT EXISTS` / dropRepository / size / provider 取句柄与缺句柄报错。
    - **验收证据（真库，事务内播种 → 执行 → 回滚，零残留）**：4 条 chunk 覆盖「可达仓库 A（活跃/历史 commit）+ 不可达仓库 B（高分）」，用适配器实际发出的 SQL 逐条执行：
      - ① `scope=A + commit=head`，limit 2 → 只返回 `a-head`（版本收窄）；
      - ② `scope=A`，limit 2 → 返回 `a-head, a-old`；而 ③ `scope=A+B` 返回的是 **`b-one, b-two`**（B 的两条全局分数更高）——**这两条放在一起就是「过滤在 LIMIT 之前」的证明**：若先取回再过滤，② 会返回 0 条；
      - ④ `minScore=0.55` → 只剩 `a-head`（0.6），`a-old`（0.5）被查询内条件挡掉；
      - ⑤ `unlink(commit-old)` → 删 1 条 link + GC 掉 1 行 chunk（`a-old`），剩 3 chunk / 3 link；重复写入同一内容仍然 4/4（幂等）。
    - **真库测试抓到一个真 bug**：`unlink` 的 GC 语句复用了 link 语句的 `params` 数组（含 commitId），而 GC 只引用 `$1` → PG 报 `bind message supplies 2 parameters, but prepared statement requires 1`。假执行器永远发现不了这类问题（它不看参数与占位符的对应关系）。已修：两条语句各自编号、各自参数数组，并补了断言。
    - 诚实边界：`minScore` 的**默认值仍未定**（Phase 7 用 golden set 定）；`search` 目前只支持单一 embedding 模型过滤（多模型并存是 V1+ 的课题）；分词列（`search_text` / `tokenizer_version`）由 P4-5 负责写入，本次 upsert 不碰它们（约束允许两者同时为 NULL）。
    - rag 215 → 239 例；全仓 429 例。
...
**下一个任务：** P4-5 稀疏索引适配器（`pg-fts-jieba` 写入 + tsquery 查询）
...
| 2026-09-24 | **Phase 4 开始：P4-1 完成**。新增 `SqlExecutor` 端口（`contracts/ports.ts`）与新 subpath `@apigent/rag/adapters`：`pg-document-source`（六段 SQL + 组装）+ `document-text`（渲染规则，独立可测）。产出 L0/L1/L2/L3 四层文档，`$ref` 指向的模型定义内联展开；「有 / 无业务上下文」两条路径各有专测。**六段 SQL 已用「假执行器导出实际 SQL → 纯 node 在真库事务内播种执行 → 回滚」验证**（不用 vitest 连库：其 `require` interop 无法 `new Client()`，且给 rag 加 `pg` 依赖是架构禁忌）。rag 153 → 187 例，全仓 373 例。文档：`rag-package.md` §2.2 subpath 表 + §4.4 实现约定。                                                                                                                                                                                                               |
| 2026-09-24 | **P4-2 完成（chunker）**：用户定案**不翻译**（中英互查交给跨语言 embedding，等 Phase 7 数据再议），因此「中英双 chunk」改写为「语言是内容属性」。`chunk_key` = `document.id`（切分加 `#n`），**修订 P0-4 的 `:lang` 后缀**（语言是内容不是身份，放 key 会随判定翻转抖动）。新增 `Chunker`/`RagChunk` 契约与 `hierarchicalChunker`/`fixedChunker`/`chunkerForStrategy`；`RagServiceOptions.chunker` 必填（不给「缺省＝不分块」的静默兜底）；`ChunkStrategyName` 与 core 的 `ChunkStrategy` 加编译期同一性断言（收口 F5）。切分规则：section 优先、每块重复标题、超长按行、单行才硬切且只首片带标题。rag 187 → 202 例，全仓 388 例。                                                                                                                                                           |
| 2026-09-24 | **P4-3 完成（embedding）**：`adapters/openai-compatible-embedder.ts` —— `openAICompatibleEmbedder()`（通用，批次 10 / 并发 2 / 逐条维度校验 / 失败带「模型+批次+条数」且不返回部分结果）+ `qwenEmbedder()`（用 core/ai 的兼容 provider 造模型，身份 `qwen:<model>`）；`packages/rag` 新增 `ai` 依赖。真 API 冒烟：v4 免费额度用尽（错误语义当场验证），改用 v3 得 3 向量 / 全 1024 维 / tokens=37；并把「跨语言相似度 0.7412 > 同语言不同主题 0.5851」记下来，支持 P4-2「不翻译」的决策。rag 202 → 215 例。                                                                                                                                                                                                                                                                                  |
| 2026-09-24 | **P4-4 完成（pgvector 稠密索引）+ minScore 配置槽**：用户定案**加槽但默认不设阈值**（值留给 Phase 7 的 golden set），四处同步 + 4 条配置用例。`adapters/pgvector-index.ts` 落地 `upsert/search/unlink/dropRepository/size`：内容寻址写入（`on conflict (repo, key, hash)`）、link 用 `insert … select …`、版本收窄用 `exists` 而非 JOIN、`fields` 摊平进 metadata 以保住「filters 在 SQL 内」、org 快照回落查库并缓存、分数从字符串转 number。**真库验证**（事务 + 回滚）证明「过滤在 LIMIT 之前」：scope=A 时仍拿到 A 的两条，而同 scope 加上 B 时返回的是 B 的两条更相似结果；minScore 在查询内挡掉低分行；unlink 的 GC 精确删 1 行。**真库测试抓到真 bug**：unlink 的 GC 语句复用了 link 的参数数组 → 占位符数量不匹配（假执行器发现不了），已修并补断言。rag 215 → 239 例，全仓 429 例。 |
