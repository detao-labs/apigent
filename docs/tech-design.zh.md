...
**key 的 scopes 不携带内容权限。** 它只说明*这把 key 能发起哪类调用*（`api:read` / `mcp:search` …）。能看到哪些仓库，由 **key 所属用户**的 `repository_members` 行与组织角色决定，逐请求解析，与浏览器会话走同一套逻辑。有效权限是两者的**交集**：只有 `mcp:search` 的 key，对一个没有仓库角色的用户依然什么都看不到；反过来，用户有仓库权限但 key 缺 scope，也调不动。这对尚未实现的 `verifySecretKey()` 有两条约束：(a) 必须拒绝**所属用户已被禁用或删除**的 key，否则禁用账号后其机器凭据仍然可用；(b) 只返回 `userId` + `scopes`，内容权限交给常规 RBAC 路径，不要在校验里重造一套。
...
**进一步收窄：`repository_ids`。** scopes 管*哪类调用*，`repository_ids` 管*哪些仓库*，两者各自独立收窄。选择器按组织分组展示 owner 可访问的仓库，但**只能勾选仓库**——组织行只是分组与"全选"的便捷操作，本身不构成范围。这样真相只有一个（一串仓库 id），也不会出现"选了组织，之后新增的仓库算不算"这类问题。

让交集保持安全的三条规则：

1. **空 = 不限制**：已有 key 行为不变，这一列是纯加法（迁移 `0004`）。UI 不依赖这条隐含知识——它用显式的"全部可访问仓库 / 指定仓库"二选一，选后者必须至少勾一个。
2. **服务端校验选择范围**：必须是调用者当前可访问集合的子集，选了打不开的仓库直接拒绝（`invalid-repositories`），不会存下来等以后再说。key 永远只能收窄 owner 的权限。
3. **范围可编辑**（`PATCH /api/keys/:id`）：否则"多加一个仓库"就得吊销并重新签发，把新密钥重新分发到 Cursor/CLI。清空选择即恢复"不限制"。
4. **删除仓库时会清理指向它的范围。** 数组元素无法加外键，数据库不会替我们做。当某个 key 的白名单被摘空时，**直接吊销该 key**，而不是留一个空数组——因为"空 = 不限制"，留着就等于把"只看这一个仓库"的 key 静默放宽成"全部可访问"。
...
**自我保护。** 管理员永远不能对**自己**下手：禁用、删除、撤销自己的管理员权限都会被拒绝（`self-not-allowed`，HTTP 400），与当前有几个管理员无关。理由是这条规则的不对称性——处置另一个管理员是有意为之，而把自己锁在门外几乎总是误操作，且控制台内没有任何自救路径。UI 会在自己那一行禁用这些按钮，但真正的约束在 API 层。
...
rag:
  ...
  vectorStore:
    ...
    port: 19530
  searchStore:
    provider: pg-fts-jieba # 也接受 "pg-fts-bigram" | "pg-fts-simple" | "none"
  retrieval:
    reranker:
      provider: cohere
```
