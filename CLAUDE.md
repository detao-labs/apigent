...
| Auth          | Custom credentials (email + password) with HMAC-SHA256 signed session cookies — no NextAuth.js. **Two planes, two cookies, two secrets:** Platform uses `apigent_session` signed with `auth.secret`; Admin uses `apigent_admin_session` signed with `auth.adminSecret`. Tokens carry `aud` and `verifySessionToken(token, scope)` requires the scope explicitly, so neither app can consume the other's cookie even though cookies are shared across ports on the same host. |
...
- `apps/admin` — Admin Webapp (Next.js SSR, port 3001): platform-admin sign-in (`/login`) plus a capability-gated `(authed)` shell; the dashboard / audit / users / settings pages are still placeholders.
...
**Platform (admin) access:** the Admin Webapp is gated by `admin_members` — one row means "this user may sign in". Login is separate from Platform (own cookie, own secret — see the Auth row above), the guard lives in `apps/admin/src/app/(authed)/layout.tsx`, and capability checks go through `assertAdminCapability()` in `@apigent/server/authz`. Granting another admin from inside the app (the `admin:admins:manage` UI) is not built yet — use the CLI command above.

**Known authorization gaps** (full list and rollout order in `docs/tech-design.md` §5.4.8): the Admin Webapp's pages are still placeholders, and SecretKey generation/verification is unimplemented — the settings page buttons are disabled and nothing validates a key today.
