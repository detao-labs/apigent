...
**Known authorization gaps** (full list and rollout order in `docs/tech-design.md` §5.4.8): the `imports` / `contexts` / `versions` services accept a `repoId` without checking the caller's role, so any signed-in user can act on any repository; `repo_permissions` has no write path, so repo members cannot be managed; `apps/admin` has no authentication at all; and `operation_logs` is unwired. SecretKey generation/verification is also unimplemented — the settings page buttons are disabled and nothing validates a key today.
