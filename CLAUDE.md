...
**Secret keys:** issue / list / revoke ship (`@apigent/server/keys`, surfaced at `/settings/keys`; keys belong to a user, plaintext is returned once and only a SHA-256 hash is stored). **Verification is deliberately not written yet** — its first consumer is the MCP Gateway, which does not exist, so `verifySecretKey()` will land together with that endpoint (docs/tech-design.md §3.10).
