...
export async function RepoForbidden({ owner }: { owner?: RepoLoadOwner | null }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center py-24 text-center">
      ...
      <p className="mb-2 max-w-md text-sm text-muted-foreground">{t("forbiddenDesc")}</p>
      {owner && (
        <div className="mb-6 flex flex-wrap items-center justify-center gap-1.5 text-sm">
          ...
          <span className="font-mono text-xs text-muted-foreground">{owner.email}</span>
