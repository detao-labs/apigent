...
export function DataModelList({ models }: { models: RepoDataModel[] }) {
  const t = useTranslations("repos.detail");
  const [query, setQuery] = React.useState("");
  const [selectedId, setSelectedId] = React.useState<string | null>(models[0]?.id ?? null);
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <p className="text-sm text-muted-foreground">{t("schemasNoResults")}</p>
              <button
                type="button"
                onClick={() => setQuery("")}
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-md px-2 py-1.5 text-left transition-colors ${
                      selected?.id === m.id ? "bg-accent" : "hover:bg-accent/60"
                    }`}
                  >
                    ...
                    <span className="mt-0.5 flex items-center justify-between gap-2 text-xs text-muted-foreground">
                      ...
                      <span className="shrink-0 tabular-nums">{propertyCount(raw)}</span>
                    </span>
                  </button>
                );
              })}
