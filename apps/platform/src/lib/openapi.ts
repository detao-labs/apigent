    ...
    components: {
      securitySchemes: {
        sessionCookie: {
          ...
          description:
            "HttpOnly session cookie issued by Auth.js (POST /api/auth/callback/credentials). Sign-in and sign-out are Auth.js endpoints; registration is POST /api/register.",
        },
      },
    },
    paths: {
      "/api/register": {
        post: {
          ...
          summary: "Register a new user (sign in afterwards via Auth.js)",
          requestBody: {
            required: true,
            content: {
